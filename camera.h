...
public:
    ...
    void ProcessMouseScroll(float yoffset)
    {
        ...
        if (Zoom < 1.0f) {
            std::cout << "Scrolled in all the way. Minimum speed set" << std::endl;
            Zoom = 2.0f;
            MovementSpeed = 0.5f; // Minimum speed set.
        }
        if (Zoom > 15 && Zoom < 30) {
            MovementSpeed = SPEED; // Set default speed in the midway point.
        }
        if (Zoom > 45.0f) {
            std::cout << "Scrolled out all the way. Maximum speed set" << std::endl;
            Zoom = 45.0f;
            MovementSpeed = 7.0f; // Maximum speed set.
        }
    }
